...
namespace HMDGazeAnalyzing {
    public class HMDGazeReplayer : MonoBehaviour
    {
        ...
        [Tooltip("The amount of data points the replayer will increment by each frame. Higher numbers means less accuracy but more speed.")]
        [Range(1, 10)]
        ...
        private void Update()
        {
            ...
            if (initialized)
            {
                //Exit the loop if there's no data to read.
                if(currentData == null)
                {
                    initialized = false;
                    replaying = false;
            ...
            {
                //Don't show the point if the replay isn't enabled.
        ...
        private void UpdateGazeIndex()
        {
            ...
            {
                int whileKiller = 0;
                while (SyncTime() && dataIndex < dataCount-1)
                {
        ...
        {
            dataIndex += frameSkip * (rewinding ? -1 : 1);
